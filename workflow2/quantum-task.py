...
rank = comm.Get_rank()
size = comm.Get_size()
print(f"from quantum: rank = {rank}, out of {size}")

all_R0 = np.linspace(5.0, 6.0, size)
...
params['R0'] = all_R0[rank] # each rank uses different parameter

import json
with open('params-' + str(rank) + '.json', 'w') as f:
    json.dump(params, f)
#
...
t2 = time()
qfile = 'quantum-data' + str(rank) + '.pkl'
pickle.dump((L1, L2, coeff), open(qfile, 'wb'))
...
t3 = time()
# spawn a process via MPI to do the postprocessing
worker = MPI.COMM_SELF.Spawn(cmd, './classical-task.py', 1)
t4 = time()
print(f"Master Time for spawning : {t4 - t3}")
worker.send(L1, dest=0, tag=99)
worker.send(L2, dest=0, tag=98)
worker.send(coeff, dest=0, tag=97)
worker.send(rank, dest=0, tag=12)

t5 = time()
print(f"Master Time for sending data : {t5 - t4}")
...
t6 = time()
print(f"Master Time for waiting : {t6 - t5}")
